...
template <> const char* HashMap<long>::str(void* value) { return UTIL_VarArgs("%ld", *(long*)value); }
...
template <> const char* HashMap<unsigned long>::str(void* value) { return UTIL_VarArgs("%lu", *(unsigned long*)value); }
// On LP64 systems (Linux x86_64) `long`==`int64_t` and `unsigned long`==`uint64_t`,
// so providing distinct specializations for {int,uint}64_t would be a redefinition.
#if defined(_WIN32) || defined(__ILP32__) || (defined(__SIZEOF_LONG__) && __SIZEOF_LONG__ < 8)
template <> const char* HashMap<int64_t>::str(void* value) { return UTIL_VarArgs("%lld", (long long)*(int64_t*)value); }
template <> const char* HashMap<uint64_t>::str(void* value) { return UTIL_VarArgs("%llu", (unsigned long long)*(uint64_t*)value); }
#endif
